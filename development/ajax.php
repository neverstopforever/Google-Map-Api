...
if (isset($_POST["func"]) && ($_POST["func"]=='saveRoute')) { 
    ...
    $schoolName = $_POST['schoolName'];
    $userName = $_POST['userName'];
    $leg_distance = $_POST['leg_distance'];
    $stopNumber = $_POST['stopNumber'];
    $busStopId = $_POST['busStopId'];
    $group_number = $_POST['group_number'];
    $route_name=$_POST['route_name'];
    $current_route_number=$_POST['current_route_number'];
    $resultId;
    
       $sql="SELECT * FROM `school_routes` WHERE `user_name`='$userName' AND `school_name`='$schoolName' AND `route_name`='$current_route_number' ORDER BY `id` DESC";
       $result = mysqli_query($db, $sql); $sql="";
       $resultRow = mysqli_fetch_array($result);
       $resultId=$resultRow;
     
       if(!isset($resultRow['id'])){
           //die("ddddd");
             $sql ="INSERT INTO `school_routes`(`id`,`user_name`, `school_name`,`route_name`,`group_number`,`start_time`,`end_time`) VALUES('','".$userName."','".$schoolName."','".$current_route_number."','".$group_number."','','')";
             
             mysqli_query($db, $sql);
        
             $getId = mysqli_query($db, "SELECT * FROM `school_routes` WHERE route_name ='$current_route_number'");
             $resultId = mysqli_fetch_array($getId);
       } 
        
        
            $sql="SELECT * FROM `school_routes_stops` WHERE `user_name`='$userName' AND `school_name`='$schoolName' AND `bus_stop_id`='$busStopId' ORDER BY `id` DESC";
            $result = mysqli_query($db, $sql); $sql="";
            $resultStop = mysqli_fetch_array($result);
            
            if(!isset($resultStop['bus_stop_id'])){
                $query ="INSERT INTO `school_routes_stops`(`user_name`, `school_name`,`route_id`,`route_name`,`bus_stop_id`,`stop_number`,`leg_duration`,`leg_distance`) VALUES('".$userName."','".$schoolName."','".$resultId[0]."','".$route_name."','".$busStopId."','".$stopNumber."','','".$leg_distance."')";
                $result = mysqli_query($db, $query);
            }
